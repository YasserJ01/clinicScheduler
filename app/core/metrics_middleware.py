...
class MetricsMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        metrics_collector.increment_request(request.method, endpoint, response.status_code)
        metrics_collector.observe_duration(request.method, endpoint, duration)

        if request.method == "POST" and request.url.path.startswith("/api/v1/appointments"):
