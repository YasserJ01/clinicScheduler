...
class TestBookAppointment:
    def test_book_appointment_success(self, http_client, auth_headers, patient_id, seeded_doctor_id, future_time_slot):
        resp = http_client.post(
            "/api/v1/appointments",
            headers=auth_headers,
            json={"doctor_id": seeded_doctor_id, "patient_id": patient_id, "time_slot": future_time_slot},
        )
    ...
    def test_book_appointment_conflict(self, http_client, auth_headers, patient_id, seeded_doctor_id):
        import uuid
        time_slot = f"2027-01-15T14:00:00Z"
    ...
    def test_book_appointment_invalid_doctor(self, http_client, auth_headers, patient_id, future_time_slot):
        resp = http_client.post(
            "/api/v1/appointments",
            headers=auth_headers,
            json={"doctor_id": 99999, "patient_id": patient_id, "time_slot": future_time_slot},
        )
    ...
    def test_book_appointment_invalid_patient(self, http_client, auth_headers, seeded_doctor_id):
        import uuid
        unique_slot = f"2027-06-15T16:00:00Z-{uuid.uuid4().hex[:6]}"
        resp = http_client.post(
            "/api/v1/appointments",
            headers=auth_headers,
            json={"doctor_id": seeded_doctor_id, "patient_id": 99999, "time_slot": "2027-06-15T16:00:00Z"},
        )
    ...
    def test_book_appointment_malformed_timeslot(self, http_client, auth_headers, patient_id, seeded_doctor_id):
        resp = http_client.post(
            "/api/v1/appointments",
            headers=auth_headers,
    ...
    def test_book_appointment_chaos_trigger(self, http_client, auth_headers, seeded_doctor_id):
        resp = http_client.post(
            "/api/v1/appointments",
            headers=auth_headers,
            json={"doctor_id": seeded_doctor_id, "patient_id": 999, "time_slot": "2027-01-15T15:00:00Z"},
        )
        assert resp.status_code == 503
        data = resp.json()
        assert "CHAOS" in data["detail"]

    def test_book_appointment_unauthenticated(self, http_client, patient_id, seeded_doctor_id, future_time_slot):
        resp = http_client.post(
            "/api/v1/appointments",
            json={"doctor_id": seeded_doctor_id, "patient_id": patient_id, "time_slot": future_time_slot},
        )
    ...
    def test_node_id_reflects_hostname(self, http_client, auth_headers, patient_id, seeded_doctor_id):
        ...
        resp = http_client.post(
            "/api/v1/appointments",
            headers=auth_headers,
            json={"doctor_id": seeded_doctor_id, "patient_id": patient_id, "time_slot": unique_slot},
        )
        assert resp.status_code == 201
        data = resp.json()
        assert data["node_id"] is not None
...
class TestListAppointments:
    ...
    def test_list_appointments_ordered_by_time(self, http_client, auth_headers, patient_id, seeded_doctor_id):
        resp = http_client.get("/api/v1/appointments", headers=auth_headers)
        assert resp.status_code == 200
