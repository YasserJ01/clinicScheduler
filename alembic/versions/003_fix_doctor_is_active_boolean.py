...
def upgrade():
    op.execute(
        "ALTER TABLE doctors ADD COLUMN is_active_bool BOOLEAN NOT NULL DEFAULT TRUE"
    )
...
def downgrade():
    op.execute(
        "ALTER TABLE doctors ADD COLUMN is_active_str VARCHAR(10) NOT NULL DEFAULT 'true'"
    )
    op.execute(
        "UPDATE doctors SET is_active_str = CASE WHEN is_active THEN 'true' ELSE 'false' END"
    )
    op.execute("ALTER TABLE doctors DROP COLUMN is_active")
